...
功能：
1. 支持输入任意日期范围（如 2025-10-01 到 2025-12-30）
2. 对范围内的每一天执行选股
3. 追踪 30 日后表现
4. 输出详细分析报告

使用方法:
    python daily_backtest.py --start 2025-10-01 --end 2025-12-30
    python daily_backtest.py --start 2025-10-01 --end 2025-12-30 --top-n 10 --hold-days 30
"""
import sys
import os
import json
...
class DailyBacktestStrategy(BowlReboundStrategy):
    ...
    def rank_stocks_by_similarity(self, stock_data_dict, target_date):
        ...
        for code, (name, df) in stock_data_dict.items():
            ...
            # 使用截至 target_date 的历史数据
            historical_df = df[df['date'] <= target_pd].copy().reset_index(drop=True)

            if len(historical_df) < 25:
                continue

            # 检查 target_date 是否有数据
            target_row = historical_df[historical_df['date'] == target_pd]
            if target_row.empty:
                # 找最接近的日期
                target_df = historical_df[historical_df['date'] <= target_pd].head(1)
                if target_df.empty:
                    continue
                actual_date = target_df['date'].iloc[0]
            else:
                actual_date = target_pd

            # 获取最近 25 天数据（按日期正序排列）
            candidate_df = historical_df.head(25)
        ...
        for i, cand in enumerate(candidates):
            cand['rank'] = i + 1
            cand['category'] = 'bowl_center'  # 简化分类
            cand['reasons'] = [f"匹配{cand['best_match_case']}"] if cand['best_match_case'] else []
...
class DailyBacktester:
    ...
    def __init__(self, config):
        ...
        self.strategy = DailyBacktestStrategy()
    ...
    def _calculate_return_and_max_gain(self, code, buy_date, hold_days, cached_df=None):
        ...
        if sell_rows.empty:
            # 找最接近的卖出日
            sell_df = df[df['date'] >= pd.to_datetime(sell_date)].head(1)
            if sell_df.empty:
                return None, None, None, None, None, None
        ...
        sell_idx = df_sorted[df_sorted['date'].dt.date == actual_sell_date]
    ...
    def run_backtest(self):
        ...
        selection_dates = self._get_date_range()
        print(f"\n回测天数：{len(selection_dates)}天")
        ...
        for idx, sel_date in enumerate(selection_dates, 1):
            # 跳过周末（可选）
            if sel_date.weekday() >= 5:  # 周六=5, 周日=6
                continue
    ...
    def _save_results(self, all_results, daily_summary):
        ...
        for r in all_results:
            simple_rows.append({
                ...
                '匹配案例': r['best_match_case'] or '-',
                '匹配日期': r['best_match_date'] or '-',
                '类别': r['category'],
                '推荐理由': ' | '.join(r['reasons']) if r['reasons'] else '-',
                '买入价': r['buy_price'],
                '卖出价': r['sell_price'],
...
def main():
    parser = argparse.ArgumentParser(description='每日回测 - B1 完美图形匹配版')
